...
| エージェント | 責務 | 判定 |
|---|---|---|
| **receipt-agent** | 領収書 ID から金額・日付・店舗・カテゴリを取り出す | しない |
| **policy-agent** | 金額とカテゴリを経費規程と照合する | する |
| **expense-orchestrator** | 上の2体に委譲し、結果が揃ってから報告する | 集約のみ |
...
agents-cli の単位はプロジェクト。3体なので**リポジトリに3プロジェクト**が並ぶ。

```
receipt-agent/            agents-cli プロジェクト（公開側）
policy-agent/             agents-cli プロジェクト（公開側）
expense-orchestrator/     agents-cli プロジェクト（呼ぶ側）
Makefile                  3つを束ねる薄いラッパ
```
...
listen するのは 18000 / 18001 / 18002 の3つだけ。
...
3つとも `agents-cli scaffold create` で生成した:
...
for i in $(seq 1 60); do
  ...
  curl -sf localhost:18002/a2a/app/.well-known/agent-card.json >/dev/null && exit 0
  sleep 1
done
...
```
[agent]: 領収書（R-1003）の内容は以下の通りです。
- 領収書ID: R-1003 / 日付: 2026-08-22 / 店舗: ホテルグランデ
- カテゴリ: 宿泊 / 金額: 45,000円

[agent]: 規程違反です（宿泊の上限15,000円を超過：45,000円）。
```
...
root_agent = Agent(name="expense_orchestrator", model=..., sub_agents=sub_agents,
    instruction="領収書の内容確認は receipt_agent に、規程判定は policy_agent に委譲し、"
...
入力: `R-1001 と R-1003 の経費をチェックして`

```
expense_orchestrator
  ├─→ receipt_agent  extract_receipt("R-1001") → 12800円 / 会食
  │                  extract_receipt("R-1003") → 45000円 / 宿泊
...
> playground の trace ビューで、どのサブエージェントにどのツール呼び出しが
> 飛んだかを実際に確認できる
...
gcloud agent-registry services create "${SERVICE_NAME:-policy-agent}" \
  --project="$GOOGLE_CLOUD_PROJECT" \
  --location="${GOOGLE_CLOUD_LOCATION:-us-central1}" \
  --display-name="${SERVICE_NAME:-policy-agent}" \
  --agent-spec-type=a2a-agent-card \
  --agent-spec-content=@/tmp/agent-card.json
```

> **`agents-cli publish` では登録できない**（publish の対象は Gemini Enterprise のみ）。
> ここは gcloud を使う。カード上限 10KB
...
sub_agents = _registry_agents() if os.environ.get("USE_AGENT_REGISTRY") == "1" \
             else _local_agents()
```
...
# make lint / make test — 3プロジェクトに流す
...
| 変数 | 置き場所 | 用途 |
|---|---|---|
| `WEB_PORT` / `RECEIPT_PORT` / `POLICY_PORT` | make 引数 | **listen する**ポート（18000/18001/18002） |
| `APP_URL` | Makefile が渡す | **カードに載る URL**（罠①） |
| `RECEIPT_AGENT_URL` / `POLICY_AGENT_URL` | 環境 | 呼ぶ側の解決先 |
| `ADK_MODEL` | 各 `.env` | 既定 `gemini-3.7-flash`。3プロジェクト共通 |
...
| `AGENT_BASE_URL` / `SERVICE_NAME` | ルートの `.env` | `gcp-registry` で必要 |
